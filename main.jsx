...
function Nav({page,go,cartQty,openCart}){
  ...
  return(
    <nav style={{position:'fixed',top:0,left:0,right:0,zIndex:1000,background:'rgba(251,240,238,0.96)',backdropFilter:'blur(14px)',borderBottom:`1px solid ${BORDER}`,height:62,display:'flex',alignItems:'center',padding:'0 24px',justifyContent:'space-between'}}>
      <div style={{position:'absolute',bottom:0,left:0,right:0,height:1,background:`linear-gradient(90deg,transparent,${RG},${RG_LT},${RG},transparent)`,backgroundSize:'300% 100%',animation:'shimmer 4s linear infinite',opacity:0.7}}/>
      {/* Logo */}
      <button onClick={()=>go('home')} style={{background:'none',border:'none',cursor:'pointer',textAlign:'left',padding:0}}>
        <div style={{fontFamily:'Cormorant Garamond,serif',fontSize:19,color:BURG,letterSpacing:0.5}}>
          The <span className="shimmer-text" style={{fontStyle:'italic'}}>Vial Vault</span>
        </div>
        <div style={{fontSize:7,letterSpacing:3.5,color:MAUVE,textTransform:'uppercase',fontFamily:'Raleway,sans-serif',fontWeight:600}}>Premium Peptides · SC</div>
      </button>
      {/* Desktop */}
...
function ProdCard({p,addCart}){
  ...
  return(
    <div className="product-card" style={{background:WHITE,border:`1px solid ${BORDER}`,display:'flex',flexDirection:'column',boxShadow:'0 2px 10px rgba(0,0,0,0.04)'}}>
      <div style={{height:3,background:`linear-gradient(90deg,${RG},${RG_LT},${RG})`,backgroundSize:'200% 100%',animation:'shimmer 4s linear infinite'}}/>
  ...
  return(
    <div style={{paddingTop:62}}>
      ...
      <div style={{background:WHITE,borderTop:`1px solid ${BORDER}`,borderBottom:`1px solid ${BORDER}`}}>
        <div style={{maxWidth:1060,margin:'0 auto',display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(200px,1fr))'}}>
      ...
      {/* Featured */}
      <div style={{maxWidth:1060,margin:'0 auto',padding:'68px 24px'}}>
        ...
        <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(255px,1fr))',gap:18,marginBottom:32}}>
...
function Products({addCart}){
  ...
  return(
    <div style={{paddingTop:62,minHeight:'100vh',background:BG}}>
      <div style={{background:`linear-gradient(155deg,${BLUSH} 0%,${BG} 100%)`,padding:'44px 24px',textAlign:'center',borderBottom:`1px solid ${BORDER}`}}>
